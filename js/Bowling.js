...
let tabPosQuillesY = [0.15,-0.15,0.40,-0.40,0.3,0,-0.3,0.15,-0.15,0];
...
function init() {
    ...
    let lumiere = new THREE.PointLight( 0x000000, 1, 0 );
    lumiere.position.set(-7, 0,0 );
    scene.add(lumiere)
    ...
    function creerPiste() {
        let piste = new THREE.TextureLoader().load('../textures/sol.png');
        let materielPiste = new THREE.MeshBasicMaterial({map: piste});
        let geometriePiste = new THREE.BoxGeometry(20,1,0.1);
        geometriePiste.castShadow = true; //default is false
        return new THREE.Mesh(geometriePiste, materielPiste);
    ...
    function lancerBoule(x,y) {
        if(x<=-7){
            for (let i=0;i<10;i++){
                let dist = Math.sqrt(Math.pow((x-tabPosQuillesX[i]),2)+Math.pow((y-tabPosQuillesY[i]),2)); //distance entre le centre la boule et le centre de la quille
                if(dist<0.45){
...
function finJeu(){
    ...
    remiseAzero(1)
    if(score1>score2){
        tourEquipe.textContent = "L'équipe 1 a gagné !";
        alert("L'équipe 1 a gagné");

    }
    else if(score1<score2){
        tourEquipe.textContent = "L'équipe 2 a gagné !";
        alert("L'équipe 2 a gagné");

    }
    else {
        tourEquipe.textContent = "Egalité";
        alert("Egalité !");
    }
